...
os.makedirs('mc_cv_l3', exist_ok=True)

# Load and prepare data
df = pd.read_csv('data/data_level_3.csv')
...
formula = (
    ...
    "access + ownership + simple_shape + is_marked + is_south_forest_steppe "
)
...
summary.to_csv('mc_cv_l3/summary_metrics.csv')
...
fig.tight_layout()
plt.savefig('mc_cv_l3/boxplots_metrics.png')
plt.close(fig)
...
for ax, metric in zip(axes.flatten(), ['R2', 'RMSE', 'MAE', 'MSE']):
    ...
    ax.legend()
fig.tight_layout()
plt.savefig('mc_cv_l3/density_metrics.png')
plt.close(fig)
...
plt.legend()
plt.tight_layout()
plt.savefig('mc_cv_l3/scatter_r2.png')
plt.close()

# Save metrics data
df_train.to_csv('mc_cv_l3/train_metrics.csv', index=False)
df_test.to_csv('mc_cv_l3/test_metrics.csv', index=False)
summary
